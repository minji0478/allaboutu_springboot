...
import java.util.List;
    ...
    public interface NoticeRepository extends JpaRepository<Notice, Long> {
        ...
        Page<Notice> findByNoticeTitle( @Param("title") String keyword,Pageable pageable);

        @Query("SELECT n FROM Notice n WHERE LOWER(n.noticeContents) LIKE LOWER(CONCAT('%', :contents, '%'))")
        Page<Notice> findByNoticeContents (  @Param("contents") String keyword,Pageable pageable);
